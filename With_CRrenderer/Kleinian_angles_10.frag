...
float  JosKleinian(vec3 z)
{
	...
	for (int i = 0; i < Box_Iterations ; i++) 
	{
		...
		if (FourGen){
			z.x=z.x+b/a*z.y;
			z = wrap(z, vec3(2. * Tx, a, 2. * Tzz), vec3(- Tx, 0., - Tzz));
			z.x=z.x-b/a*z.y;
		}
		else
			z.xz = wrap(z.xz, vec2(2. * Tx, 2. * Tzz), vec2(- Tx, - Tzz));
...
#preset Pluton
FOV = 0.71698
Eye = 0.00445197,0.191546,-1.00481
Target = -0.036676,0.0171414,0.0986019
Up = 0.135631,0.990069,0.0369799
FocalPlane = 1
Aperture = 0
InFocusAWidth = 0
ApertureNbrSides = 5
...
ShadowSoft = 10
BaseColor = 0.552941,0.552941,0.552941
OrbitStrength = 0.31667
X = 0.203922,0.521569,1,0.32558
Y = 1,0.294118,0.0823529,1
Z = 0.141176,0.776471,0.196078,-0.4186
R = 0.262745,0.482353,1,0.3647
BackgroundColor = 0.270588,0.403922,0.6
GradientBackground = 0
CycleColors = false
Cycles = 6.61245
