...
uniform float HF_Scatter; slider[0.0,0.,100.0]
//Anisotropy of the fog.
uniform vec4 HF_Anisotropy; color[-1,1,1,0,0,0] 
...
float fogAmount(vec3 P0, vec3 P1){//Modified from: http://www.iquilezles.org/www/articles/fog/fog.htm
	...
	A=(1.0-exp1(-A))/A;//TODO: 1-exp(x) is not accurate. There should be an special function for that case lake in c / c++'s
...
vec3 getVdirAO(vec3 dir){//dir is supposed to be normalized
	...
	r.x *= 2. * PI;
	r.y  = 2. * (r.y - 0.5);
	float f = sqrt( 1. - r.y * r.y);
	vec3 s = vec3( f * cos(r.x) , f * sin(r.y), r.y); //Uniform sampling over a unit radius sphere.
	return normalize(s * 0.5 * coneApertureAO + dir); // scale and add to dir (supposed to be the normal)
}
vec3 getVdirAO1(vec3 dir){//dir is supposed to be normalized
	//return dir;
...
vec3 ptLightGlow3(vec3 P0, vec3 P1){
	...
	//Anisotopy factor
	float cosTheta=dot(normalize(Pt-P0),normalize(LightPos-Pt));//cos(theta): the angle between light direction and view direction at the sample
	vec3 denom = 1. + HF_Anisotropy.xyz*HF_Anisotropy.xyz - 2. * HF_Anisotropy.w * HF_Anisotropy.xyz * cosTheta;
	vec3 Anie=(1. - HF_Anisotropy.xyz * HF_Anisotropy.xyz)/sqrt(denom*denom*denom);//Dropped the 1./(4.*PI)* factor. Well This renderer is not meant to be physically accurate after all. :o)
	//Scattering at sample point
...
vec3 color(SRay Ray) {
	...
	for(int i=0; i<=ReflectionsNumber;i++){
		...
		//------------------------------------------------------------------
		vec3 f0=Reflection;
		vec3 fresnel=f0+(.75-f0)*pow(1.+dot(hitNormal,Ray.Direction),5.);
		RWeight *= fresnel * FG;//modify current opacity
		//------------------------------------------------------------------
#else
