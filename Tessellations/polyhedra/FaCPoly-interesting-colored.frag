...
#info Experimenting UVW barycentric coordinates beyond [0,1]
...
uniform float U; slider[-2,1,2]

// V
uniform float V; slider[-2,0,2]

// W
uniform float W; slider[-2,0,2]
...
void init() {
	...
	pab=vec3(0.,0.,1.);
	pbc=1./sin(PI/float(Type))*vec3(scospin,0.,0.5);//normalize(vec3(scospin,0.,0.5));
	pca=2./sqrt(3.)*vec3(0.,scospin,cospin);//normalize(vec3(0.,scospin,cospin));//
