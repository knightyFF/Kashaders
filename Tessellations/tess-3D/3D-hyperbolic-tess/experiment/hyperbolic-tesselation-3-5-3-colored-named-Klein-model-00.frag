...
float DE(vec3 pos) {
	float r=length(pos);
	vec4 z4=vec4(pos,1.)*1./sqrt(1.-r*r);//Inverse stereographic projection of pos: z4 lies onto the unit 3-parabolid of revolution around w axis centered at 0.
	z4=Rotate(z4);
	z4=fold(z4);
	orbitTrap=z4;
...
vec3 baseColor(vec3 pos, vec3 normal){
	float r=length(pos);
	vec4 z4=vec4(pos,1.)*1./sqrt(1.-r*r);
	z4=Rotate(z4);
	z4=fold(z4);
